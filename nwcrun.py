...
while True:
    ...
    if udo == '':
        operation = input ('What kind of task do you want to run? "ge" for generating files, "pr" for processing log files: ')
        if operation == 'ge':
            xyzorg = input ('Type the name of the original xyz file stored in location: /pscratch/anthony_uksr/hmle231/NWchem/original_xyz_file: ')
            nwfolder = input ('Type in folder name to store the files: ')
